...
Ahora que comprendes los conceptos de Datos Abiertos, ¿qué herramientas se usan para el Código Abierto? En esta lección presentaremos algunos consejos y herramientas útiles para trabajar con código, para que puedas compartirlo y reusarlo. Aprenderemos acerca de las diversas herramientas disponibles para desarrollar, almacenar y compartir Código Abierto, desde el control de versiones hasta el software de edición de código y los contenedores. En definitiva, estas herramientas digitales existen para gestionar y alojar código, así como para fomentar la colaboración.
...
Git es muy potente y ampliamente usado. Según una encuesta de desarrolladores de Stack Overflow (2022 Developer Survey, 2022), más del 87% de las personas que desarrollan software usan Git, pero eso no significa que sea sencillo de aprender. Si Git te resulta confuso al principio, ¡no estás solo! Para una capacitación en profundidad sobre Git, consulta la lección de Software Carpentry "[Control de versiones con Git: Resumen y configuración](https://swcarpentry.github.io/git-novice/)" (enlace externo, en inglés).
...
- Para compartir tu código con otras personas (más sobre esto más adelante en este curso).
- Para que otras personas colaboren en tu código. Al subirlo a GitHub, tu código puede ser accesible para quienes quieran añadir mejoras.
...
<table>
  ...
  <tbody>
    <tr class="odd">
      ...
      <td>
        Realizar un seguimiento de las solicitudes de funciones, de errores y de otras actualizaciones a través de GitHub Issues. GitHub también permite usar etiquetas y asignar personas a las tareas para ayudar a organizarlas
      </td>
    </tr>
    <tr class="odd">
    ...
    <tr class="odd">
      ...
      <td>
        Elige a quien tiene autorización para actualizar el código. Esto ayuda a que tengas la seguridad de que solo quienes tienen permiso pueden actualizar el código que compartiste en GitHub, y también a que otras personas se sientan seguras al sugerir actualizaciones sin preocuparse de que puedan sobrescribir accidentalmente el código existente.
      </td>
    </tr>
  </tbody>
</table>

Todas estas funciones son excelentes para facilitar la colaboración asíncrona entre equipos. La mayoría de los paquetes de Código Abierto científico usan GitHub para el desarrollo de su código principal. Ten en cuenta que hay muchas más funciones de GitHub que no profundizamos aquí y que permiten la colaboración, así como flujos de trabajo automatizados y mucho más. Para aprender más sobre GitHub, echa un vistazo a estas referencias:
...
### Resumen de las ventajas de usar el control de versiones y las plataformas de control de versiones
...
Un *kernel* (en español, núcleo) es el código central que constituye la base del sistema operativo de una computadora. Gestiona los recursos del sistema e interactúa con el hardware. El *kernel* es un puente entre el hardware y el software.

 **Edición de código fuente y kernels: el valor de los IDE y los kernels**

Los IDEs pueden aportar muchas herramientas útiles a tus trabajos. Ya no se trata únicamente de editar código. Los IDEs modernos y robustos pueden hacer la mayoría de las cosas que se listan aquí y aun más. Se puede usar un IDE sin ejecutar en un *kernel*; se puede usar un *kernel* sin haber desarrollado código en un IDE. Sin embargo, pueden trabajar de la mano.
...
- Dispone de una opción de "modo oscuro" que resulta más agradable a la vista durante largas sesiones de codificación.
- Proporciona resaltado sintáctico y una ventana de terminal integrada.
- También cuenta con una gran cantidad de complementos para conectarse a servidores, sistemas de control de versiones y de resolución de problemas. Dispone de varios complementos que pueden analizar el código en busca de fallas y errores y contribuyen a que el equipo codifique con un "estilo" consistente. Esto facilita el mantenimiento del código en el futuro.
...
A continuación hay un ejemplo de un desarrollador que, por accidente, escribió un signo igual cuando debía escribir dos puntos. VS Code detectó el error y, cuando el desarrollador pasó el cursor sobre el subrayado rojo, VS Code explicó cuál era el error y ofreció llevarlo a documentación adicional.
...
- Descargar los cambios de otros miembros del equipo en tu sistema local.

 **Ejemplo de IDE: RStudio – IDE**

Mientras que VS Code es un entorno de desarrollo integrado (en inglés, Integrated Development Environment, IDE) más genérico, en el que puedes usar complementos para especializarlo, también existen IDEs, como RStudio, que desde el principio tienen características especializadas para lenguajes específicos.

Las personas investigadoras que realizan análisis estadísticos tienden a usar los lenguajes de programación R y Python. RStudio tiene herramientas integradas para ese mismo propósito, incluida la visualización de datos.
...
La mayoría de los ordenadores portátiles o de escritorio que ejecutan sistemas operativos estándar (Windows, MacOS, Linux) tienen múltiples editores de texto plano preinstalados que se pueden usar para programar. Es beneficioso saber cómo usar al menos uno, porque hace que la edición de programas y archivos sea un proceso rápido.
...
Un cuaderno computacional es un entorno de computación virtual e interactivo que combina la ejecución de código, la documentación y la visualización de datos en una única interfaz. Estos cuadernos se usan ampliamente en los campos de las ciencias de datos y la programación. Ejemplos populares son Jupyter Notebooks y R Notebooks. Permiten a los usuarios escribir y ejecutar código paso a paso, proporcionando una plataforma eficiente para el análisis de datos, la investigación y la programación colaborativa, con el beneficio añadido de integrar texto enriquecido (incluidas ecuaciones), imágenes y gráficos para una documentación y una comunicación claras.
...
Jupyter Notebook es una aplicación web de Código Abierto ampliamente usada para la creación de documentos computacionales. Pero antes de sumergirnos en Jupyter Notebook, queremos dejar en claro que Jupyter Notebook es una de las muchas plataformas del ecosistema de Jupyter:
...
Usaremos Jupyter Notebook como ejemplo de un cuaderno computacional y discutiremos cómo se relaciona JupyterLab con Jupyter Notebook. La siguiente sección sobre plataformas de cómputo tratará sobre JupyterHub.
...
JupyterLab es un entorno de desarrollo interactivo basado en un navegador, compatible con Jupyter Notebook, y está diseñado para ofrecer una experiencia más flexible que incorpora muchas características útiles. Una de estas características es la integración con Git, como vimos en otros IDEs como Visual Studio Code.

Dado que Jupyter Notebooks permite integrar código con visualizaciones y texto, puede servir como herramienta para llevar adelante proyectos de investigación y crear documentos computacionales fáciles de compartir para educación, colaboración o comunicación científica. Con capacidades de texto enriquecido, como encabezados, cursivas, enlaces y mucho más, puedes crear un documento legible que incluya código ejecutable. Estas son algunas de las razones por las que JupyterLab y Jupyter Notebooks se usan ampliamente en diversas disciplinas, incluidas la investigación computacional y la ciencia de datos.
...
2. Localiza y selecciona la opción "Binder" en el desplegable del icono del cohete (en inglés, Rocket). Esto abrirá la lección como un Jupyter Notebook ejecutable que se ejecuta en la nube. Ten en cuenta que el Cuaderno Computacional puede tardar varios minutos en configurarse.
...
3. Después de cargar el Cuaderno Computacional, deberías encontrarte ahora en una interfaz de edición con un árbol de archivos y una sección de edición de los archivos del árbol.Nota -- ¡Esto realmente usa la vista de JupyterLab!

<img src="../images/media/calculatingenso2_es.jpg" style="width:100%;height:auto;" alt="Captura de pantalla con ejemplo de Notebook Paso 3. JupyterLab con el notebook «Calculating ENSO with Xarray» abierto. A la izquierda se ve el árbol de archivos y, en el centro, el área de edición del cuaderno con el título y secciones como «Overview» y «Prerequisites»." />
Créditos de imagen: [Calculating ENSO with Xarray](https://foundations.projectpythia.org/core/xarray/enso-xarray.html). (enlace externo)
...
Usamos el término "plataforma de cómputo" para referirnos a la máquina en la que se ejecuta el código. Hay muchas plataformas de cómputo diferentes que puedes elegir y cada una tiene sus ventajas y desventajas. A continuación se ofrece una descripción general de tres opciones de plataformas:
...
- Falta de transparencia en los costos. Por ejemplo, leer datos de diferentes regiones de la nube puede costar mucho dinero, pero no siempre está claro en qué región se encuentran los datos ni dónde se realizan los cálculos.
- Puede requerir conocimientos adicionales sobre cómo funciona el cómputo en la nube.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Herramienta que ayuda a quienes desarrollan software a gestionar y dar seguimiento a los cambios.
      </td>
    </tr>
  </tbody>
</table>
...
- Cómo los sistemas de control de versiones, como Git, y las plataformas, como GitHub, pueden aumentar la colaboración y la gestión del código.
...
¿Cuál de las siguientes NO es una ventaja de usar plataformas de control de versiones?
